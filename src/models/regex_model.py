...
#         # followed by an optional space
#         # this patten can repeat 2 or more times
#         r"(([A-Z][a-z]{2,}|[A-Z]{3,}|" + CONNECTING_PATTERN + r")('s)?[\ ]?){2,}",
#         # Third part is a logical lookbehind that exlcudes the last match from
#         # the second part from being one of the CONNECTING_WORDS
...
class RegexModel(Model):
    ...
    def __init__(self, config:Dict[str, str]) -> None:
        ...
        if keywords:
            ...
            regex_pattern = regex_pattern[:-1] + "|(" + keyword_pattern + "))"
    ...
    def regexify_keyword(keyword:str) -> str:
        ...
        if len(tokens) == 1:
            return "".join(list(map(RegexModel.regexify_char, keyword)))
        else:
            return " ".join(list(map(RegexModel.regexify_first_char, tokens)))
